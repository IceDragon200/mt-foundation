...
function ic:initialize(name, definition)
  ...
  self.m_definition = foundation.com.list_map(definition, function (element)
    ...
    elseif type(element) == "table" then
      ...
      if type(t) == "string" then
        ...
        if t == "*array" then
          ...
          return {name = name, type = foundation.com.binary_types.Array:new(value_type, -1)}
        -- fixed length array
        elseif t == "array" then
          local value_type = element[3]
          assert(value_type, "expected a value_type")
          ...
          return {name = name, type = foundation.com.binary_types.Array:new(value_type, len)}
        elseif t == "map" then
          local kt = element[3]
          assert(kt, "expected a key type")
          local vt = element[4]
          assert(vt, "expected a value type")
          return {name = name, type = foundation.com.binary_types.Map:new(kt, vt)}
        elseif foundation.com.binary_types.Scalars[t] then
          return {name = name, type = foundation.com.binary_types.Scalars[t]}
        else
