...
do
  ...
  function ic:initialize(data, mode)
    ...
    self.m_writable = false
    self.m_readable = false
    self:open(mode)
  end
  ...
  function ic:truncate()
    assert(self.m_writable, "expected write mode")
    self.m_data = ""
    self.m_size = 0
    self.m_cursor = 1
    self.m_lazy_data = {
  ...
  function ic:open(mode)
    ...
    self.m_mode = mode
    string_each_char(mode, function (c)
      if c == "r" then
  ...
  --- Does the same thing as scan/1, but doesn't automatically prefix the pattern with ^.
  --- Use this if you want to handle the prefixing yourself, to save time.
  ---
  --- @since "2.5.0"
  --- @spec #scan_raw(pattern: String): (String, bytes_read: Integer) | (nil, Integer)
  function ic:scan_raw(pattern)
    self:check_readable()
  ...
  function ic:scan_while(pattern)
    ...
    local k = next(result)
    if k then
      return table.concat(result), self.m_cursor - pos
    end
    return nil
  end
  ...
  function ic:calc_read_length(len)
    assert(self.m_readable, "expected read mode")
  ...
  --- Reads the buffers next available data without advancing the cursor and returns a byte.
  ---
  --- @spec #peek_byte(): (Integer, Integer)
  function ic:peek_byte()
    self:check_readable()
    return string.byte(self.m_data, self.m_cursor), 1
  end
  ...
  --- Writes the given value `count` number of times to the buffer at the current position forward.
  ---
  --- @since "2.5.0"
  --- @spec #fill(count: Integer, value: Any): (true, nil)
  function ic:fill(count, value)
    if count > 0 then
      for _ = 1,count do
        self:write(value)
      end
  ...
  --- @since "2.5.0"
  --- @spec #fill_bytes(count: Integer, value: Integer): (true, nil)
  function ic:fill_bytes(count, value)
    if count > 0 then
