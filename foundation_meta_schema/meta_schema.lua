...
--- Usage:
---
---     local schema = MetaSchema:new("MyCoolSchema", "mcs", {
---       name = {
---         type = "string",
---       },
---       amount = {
...
do
  local ic = assert(MetaSchema.instance_class)
  ...
  ---
  --- @spec #initialize(name: String, prefix: String, schema: Schema): void
  function ic:initialize(name, prefix, schema)
  ...
  --- * Compiled schema
  ---
  --- @spec #compile(basename: String): Table
  ...
  function ic:set_field(meta, basename, key, value)
  ...
  function ic:set(meta, basename, params)
  ...
  function ic:get_field(meta, basename, key)
    if self.schema[key] then
      local entry = self.schema[key]
      local field_name = (self.prefix or "") .. basename .. "_" .. key
