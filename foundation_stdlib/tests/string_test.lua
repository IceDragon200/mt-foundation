...
local case = Luna:new("foundation.com.string")
...
case:describe("string_ascii_iequals/2", function (t2)
  t2:test("can check equality of two ascii strings", function (t3)
    t3:assert(m.string_ascii_iequals("HELLO", "hello"))
    t3:assert(m.string_ascii_iequals("123FOU", "123Fou"))
    t3:assert(m.string_ascii_iequals("123 Lost & Found", "123 LOST & found"))
    t3:refute(m.string_ascii_iequals("Eggs Bacon Rice", "Apple Banana Canteloupe"))
  end)
end)

case:describe("string_ascii_icontains/2", function (t2)
  t2:test("can check that base string includes the provided pattern", function (t3)
    t3:assert(m.string_ascii_icontains("HELLO", "HeL"))
    t3:assert(m.string_ascii_icontains("HELLO", "ell"))
    t3:assert(m.string_ascii_icontains("HELLO", "Lo"))
    t3:assert(m.string_ascii_icontains("HELLO", ""))
    t3:refute(m.string_ascii_icontains("HELLO", "Nope"))
    t3:refute(m.string_ascii_icontains("HELLO", "Toolong"))
    t3:refute(m.string_ascii_icontains("", "HEL"))
  end)
end)

case:describe("string_to_boolean/1", function (t2)
  t2:test("can cast a string to a boolean", function (t3)
    t3:assert_eq(m.string_to_boolean("true"), true)
    t3:assert_eq(m.string_to_boolean("TRUE"), true)
    t3:assert_eq(m.string_to_boolean("false"), false)
    t3:assert_eq(m.string_to_boolean("FALSE"), false)
    t3:assert_eq(m.string_to_boolean("True"), nil)
    t3:assert_eq(m.string_to_boolean("False"), nil)
  end)
end)
